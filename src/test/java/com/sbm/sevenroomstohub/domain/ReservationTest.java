...
class ReservationTest {
    ...
    @Test
    void tableNumTest() throws Exception {
        Reservation reservation = getReservationRandomSampleGenerator();
        TableNum tableNumBack = getTableNumRandomSampleGenerator();

        reservation.addTableNum(tableNumBack);
        assertThat(reservation.getTableNums()).containsOnly(tableNumBack);
        assertThat(tableNumBack.getReservation()).isEqualTo(reservation);

        reservation.removeTableNum(tableNumBack);
        assertThat(reservation.getTableNums()).doesNotContain(tableNumBack);
        assertThat(tableNumBack.getReservation()).isNull();

        reservation.tableNums(new HashSet<>(Set.of(tableNumBack)));
        assertThat(reservation.getTableNums()).containsOnly(tableNumBack);
        assertThat(tableNumBack.getReservation()).isEqualTo(reservation);

        reservation.setTableNums(new HashSet<>());
        assertThat(reservation.getTableNums()).doesNotContain(tableNumBack);
        assertThat(tableNumBack.getReservation()).isNull();
    }
