<template>
  <div>
    <h2 id="page-heading" data-cy="DepartmentHeading">
      <span v-text="t$('sevenRoomsToHubApplicationApp.department.home.title')" id="department-heading"></span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="t$('sevenRoomsToHubApplicationApp.department.home.refreshListLabel')"></span>
        </button>
        <router-link :to="{ name: 'DepartmentCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="t$('sevenRoomsToHubApplicationApp.department.home.createLabel')"></span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && departments && departments.length === 0">
      <span v-text="t$('sevenRoomsToHubApplicationApp.department.home.notFound')"></span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="departments">
        <thead>
          <tr>
            ...
            <th scope="row"><span v-text="t$('sevenRoomsToHubApplicationApp.department.departmentName')"></span></th>
            <th scope="row"><span v-text="t$('sevenRoomsToHubApplicationApp.department.location')"></span></th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <template #modal-title>
        <span
          id="sevenRoomsToHubApplicationApp.department.delete.question"
          data-cy="departmentDeleteDialogHeading"
          v-text="t$('entity.delete.title')"
        ></span>
      </template>
      <div class="modal-body">
        <p id="jhi-delete-department-heading" v-text="t$('sevenRoomsToHubApplicationApp.department.delete.question', { id: removeId })"></p>
      </div>
